...
data {
  ...
  // ── Dialect group (inner interaction; sum-to-zero coded) ──────────────
  // Integer codes 1..D assigned by the R data builder.
  // Values outside {attic, doric, ionic} are collapsed to "other".
  int<lower=1>                    D;
...
parameters {
  ...
  // Global inner context shift (grand mean across dialect groups)
  real gamma_ctx;

  // Dialect-group inner deviations (raw; sum-to-zero centred)
  // delta_ctx[d] is the deviation of dialect group d from gamma_ctx.
  // The total inner shift for dialect d is gamma_ctx + delta_ctx[d].
  vector[D] delta_ctx_raw;
...
transformed parameters {
  ...
  // Identical treatment for both type and dialect inner deviations.
  vector[T] beta_type = beta_type_raw - mean(beta_type_raw);
  vector[D] delta_ctx = delta_ctx_raw - mean(delta_ctx_raw);
...
model {
  // ── Priors ───────────────────────────────────────────────────────────
  // All priors for shared parameters are identical to Model 1.
  alpha         ~ normal(0, 1);
  beta_date     ~ normal(0, 2);
  beta_type_raw ~ normal(0, 2);
  gamma_ctx     ~ normal(0, 2);
  delta_ctx_raw ~ normal(0, 2);   // same regularisation as beta_type_raw
  ...
  // The only change from Model 1 is that the inner predictor gains the
  // dialect-specific term delta_ctx[dialect[i]], computed once per
  // inscription outside the quadrature loop (it does not depend on t).
  for (i in 1:N) {
    ...
    real ctx_shift       = gamma_ctx + delta_ctx[dialect[i]];
    ...
    for (k in 1:K_u) {
      ...
      real mu2 = inv_logit(eta + ctx_shift);
      real a2  = mu2 * phi;
      real b2  = (1.0 - mu2) * phi;
...
generated quantities {
  ...
  // Total dialect-level inner shift: gamma_ctx + delta_ctx[d].
  // Direct parametric analog of delta_site[S] in Model 2.
  // Sanity check: doric >> attic > other > ionic (mirrors f_site ordering).
  vector[D] delta_ctx_total;
  for (d in 1:D) {
    delta_ctx_total[d] = gamma_ctx + delta_ctx[d];
  }
  ...
  for (i in 1:N) {
    ...
    // Posterior-predictive replications at the quadrature mid-point date
    {
      ...
      real mu2_mid = inv_logit(eta_mid + ctx_shift);
