...
parameters {
  ...
  real<lower=0> rho_t;      // temporal length scale (standardized dates)
                             //   1 unit ≈ 120 years
  vector[S] z_gp;           // non-centered weights

  // ── GP 2: inner-contrast surface (g_site) ───────────────────
  real<lower=0> sigma_ctx;  // marginal SD of the inner-contrast GP
  vector[S] z_ctx;          // non-centered weights (independent of z_gp)
}
