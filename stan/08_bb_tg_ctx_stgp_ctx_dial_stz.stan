data {
  ...
  // ── Date-uncertainty quadrature (standardized) ──────────────────────
  int<lower=1> K_u;
  vector[K_u]  u_grid;       // K_u nodes on (0, 1)
  vector[N]    t_min_std;    // standardized lower date bound per inscription
  vector[N]    t_max_std;    // standardized upper date bound per inscription
...
transformed data {
  // ── standardize site spatial coordinates ────────────────────────────
  ...
  // ── Per-site mean standardized mid-date (temporal GP coordinate) ────
  vector[S] t_site;
  {
...
parameters {
  ...
  // Dialect-group inner deviations (raw; sum-to-zero centered)
  vector[D] delta_ctx_raw;

  // Beta-binomial concenteration
  real<lower=0> phi;

  // Spatiotemporal GP hyperparameters
  real<lower=0> sigma_gp;   // marginal SD of the GP
  real<lower=0> rho_s;      // spatial  length scale (standardized coords)
  real<lower=0> rho_t;      // temporal length scale (1 unit ≈ 120 years)

  // Non-centered GP weights
  vector[S] z_gp;
}
...
model {
  ...
  sigma_gp ~ normal(0, 1);
  rho_s    ~ lognormal(0, 0.8);   // median ≈ 1 standardized spatial unit
