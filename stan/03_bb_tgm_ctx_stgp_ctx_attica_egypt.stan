...
data {
  ...
  // ── Region indicators at the site level ───────────────────────────
  // These encode the prior belief that Attica and Egypt are special.
  array[S] int<lower=0, upper=1> attica_site;
  array[S] int<lower=0, upper=1> egypt_site;
}
...
transformed data {
  ...
  // A site coded as both Attica and Egypt would silently receive a double
  // boost (gamma_attica + gamma_egypt).  Catch this at compile/run time.
  for (s in 1:S) {
    if (attica_site[s] + egypt_site[s] > 1)
      reject("Site ", s, " is coded as both attica_site=1 and egypt_site=1. ",
...
parameters {
  ...
  // Global baseline inner shift (log-odds scale)
  real gamma_ctx;

  // Region-level inner shifts for Attica and Egypt.
  // Unconstrained: the prior N(1.5, 1.0) expresses a soft belief that these
  // are positive, but the data can push them slightly negative if warranted.
  real gamma_attica;
  real gamma_egypt;
  ...
  // ── GP 2: residual inner-contrast surface (g_site) ───────────────────
  // This is now the residual surface after the Attica/Egypt region terms.
  real<lower=0> sigma_ctx;  // marginal SD of the residual inner-contrast GP
...
transformed parameters {
  ...
  // ── Region-specific inner shift at each site ────────────────────────
  vector[S] region_ctx_shift;
  for (s in 1:S) {
    region_ctx_shift[s] = gamma_attica * attica_site[s]
                        + gamma_egypt  * egypt_site[s];
  }

  // ── GP draws from a single local Cholesky (K_site not saved) ─────
  vector[S] f_site;
  vector[S] g_site;   // residual inner-contrast deviation surface
  {
    ...
    matrix[S, S] K_site;
    for (i in 1:S) {
      // Diagonal: pure variance plus jitter for numerical stability.
      K_site[i, i] = sq_sigma + 1e-6;
      for (j in (i + 1):S) {
        real kij = sq_sigma
...
model {
  ...
  // Global baseline inner shift.
  // sd=2 is chosen deliberately over the original sd=4 of the main model.
  // With sd=4, roughly 32 % of prior mass falls outside |γ| > 4 log-odds
  // (OR > 55 or < 0.02), which is implausible for a phonological context
  // effect.  sd=1 would be too tight given the absence of strong prior
  // knowledge about the global baseline direction.  sd=2 keeps 95 % of
  // the mass within a plausible range while still being weakly informative.
  gamma_ctx     ~ normal(0, 2);

  // Soft regional beliefs: the inner shift is probably larger in Attica and
  // Egypt than the global baseline, but the data may revise this.
  // N(1.5, 1.0) places 93 % of mass above zero and 5–95th percentile at
  // approximately [−0.1, 3.1] on the log-odds scale.  The parameters are
  // now unconstrained, so the sampler faces no hard boundary near zero.
  gamma_attica  ~ normal(1.5, 1.0);
  gamma_egypt   ~ normal(1.5, 1.0);
  ...
  // GP 2 (residual inner-contrast surface)
  sigma_ctx ~ normal(0, 1);
  z_ctx     ~ normal(0, 1);
  ...
  for (i in 1:N) {
    ...
    real ctx_resid    = g_site[site_id[i]];
    real ctx_region   = region_ctx_shift[site_id[i]];
    ...
    for (k in 1:K_u) {
      ...
      real mu2 = inv_logit(eta + gamma_ctx + ctx_region + ctx_resid); // inner
      real a2  = mu2 * phi;
      real b2  = (1.0 - mu2) * phi;
...
generated quantities {
  ...
  // This is the primary quantity for spatial mapping.
  vector[S] delta_site;

  // Decomposed inner shift components (saved for downstream analysis).
  // delta_site[s] = gamma_ctx + region_ctx_shift_out[s] + g_site[s]
  // region_ctx_shift is already available from transformed parameters, but
  // re-exposing it here makes it extractable by all Stan interfaces without
  // special handling of transformed parameters outputs.
  vector[S] region_ctx_shift_out;

  // mid_idx: integer-division midpoint of the quadrature grid, used as a
  // single representative date for posterior-predictive draws.
  // (%/% is Stan's integer-division operator.)
  int mid_idx = (K_u + 1) %/% 2;

  for (s in 1:S) {
    region_ctx_shift_out[s] = region_ctx_shift[s];
    delta_site[s]           = gamma_ctx + region_ctx_shift[s] + g_site[s];
  }
  ...
  for (i in 1:N) {
    ...
    for (k in 1:K_u) {
      ...
      real mu2 = inv_logit(eta + gamma_ctx + ctx_region + ctx_resid);
    ...
    // Posterior-predictive replications using the quadrature mid-point date
    {
      ...
      real mu2_mid = inv_logit(eta_mid + gamma_ctx + ctx_region + ctx_resid);
