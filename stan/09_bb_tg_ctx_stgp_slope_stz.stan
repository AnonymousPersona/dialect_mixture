...
parameters {
  ...
  vector[T] beta_type_raw;   // text-type effects (raw; sum-to-zero centered)

  real           gamma_ctx;  // scalar inner context shift
  real<lower=0>  phi;        // beta-binomial concenteration
  ...
  vector[S] z_gp;            // non-centered weights for f_site
  ...
  real<lower=0> rho_slope;   // spatial length scale of the slope GP
                              // (same units as rho_s: standardized coords)

  vector[S] z_slope;         // non-centered weights for b_site
}
