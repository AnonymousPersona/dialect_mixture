...
ensure_std_time <- function(df, t0, scale_years) {
  if (all(c("t_min_std", "t_max_std") %in% names(df))) {
    ...
    if (!isTRUE(all.equal(t0_old, t0)) ||
      ...
      warning(sprintf(
        "Data already standardized with t0=%s, scale=%s; ",
...
# Keep raw (unstandardized) versions for the time-origin
...
# as the time origin for standardization.  This value is data-
...
t_scale_years <- 100   # 1 standardized time unit = 100 years
...
# Convenience accessor: returns the correct standardized dataset
